  ...
  return (
    <PageShell>
      <section className="relative isolate overflow-hidden bg-[#0B1F3A] px-4 py-14 text-white sm:px-6 sm:py-16 lg:px-8 lg:py-20" aria-labelledby="contact-heading">
        ...
        <div className="mx-auto grid max-w-6xl gap-8 lg:grid-cols-[minmax(0,1fr)_minmax(19rem,0.42fr)] lg:items-center lg:gap-14">
          <div className="max-w-3xl">
            ...
            <h1 id="contact-heading" className="mt-4 max-w-4xl text-[2.15rem] font-bold leading-[1.1] tracking-[-0.045em] sm:text-[3rem] lg:text-[4rem]">
              Let&apos;s Build the Right Technology Solution Together
            </h1>
            <p className="mt-5 max-w-2xl text-base leading-[1.7] text-slate-200 sm:text-lg">
            ...
            <Link href="#contact-form" className="btn hero-cta-primary mt-7 w-full sm:w-auto">
              <span>Start Your Enquiry</span><ArrowRight aria-hidden="true" className="size-4" />
            </Link>
          </div>

          <aside className="rounded-[22px] border border-white/15 bg-white/[0.06] p-5 shadow-[inset_0_1px_0_rgba(255,255,255,0.08)] backdrop-blur sm:p-6" aria-label="Contact conversation assurances">
            <h2 className="text-xl font-bold tracking-[-0.025em]">A Thoughtful First Conversation</h2>
      ...
      <section id="contact-form" className="scroll-mt-24 bg-[#F7F9FC] px-4 py-12 sm:px-6 sm:py-14 lg:px-8 lg:py-16" aria-labelledby="contact-form-heading">
        <div className="mx-auto grid max-w-6xl gap-8 lg:grid-cols-[minmax(0,0.62fr)_minmax(18rem,0.38fr)] lg:gap-10">
          <div>
            <h2 id="contact-form-heading" className="sr-only">Contact form</h2>
            <ContactForm />
          </div>
          <aside className="rounded-[22px] border border-[#DCE3EA] bg-white p-6 shadow-[0_18px_48px_rgba(15,23,42,0.06)] sm:p-8">
            <h2 className="text-2xl font-bold leading-tight tracking-[-0.035em] text-[#0B1F3A]">Prefer to Contact Us Directly?</h2>
            <div className="mt-6 space-y-5 text-[#475569]">
              <div className="flex gap-3"><Mail aria-hidden="true" className="mt-1 size-5 text-[#10B981]" /><div><p className="text-sm font-bold text-[#0B1F3A]">Email</p><a className="mt-1 block break-all text-base font-semibold text-[#173B67] hover:text-[#10B981]" href={`mailto:${directEmail}`}>{directEmail}</a></div></div>
            ...
            <p className="mt-7 rounded-2xl border border-[#DCE3EA] bg-[#F8FAFC] p-4 text-sm leading-6 text-[#294A43]">Your information is treated confidentially and used only to respond to your enquiry.</p>
          </aside>
        </div>
      </section>
      ...
      <section className="bg-white px-4 py-12 sm:px-6 sm:py-14 lg:px-8 lg:py-16" aria-labelledby="next-heading">
        <div className="mx-auto max-w-6xl">
          <div className="max-w-3xl"><p className="text-xs font-bold uppercase tracking-[0.2em] text-[#10B981]">PROCESS</p><h2 id="next-heading" className="mt-3 text-[2rem] font-bold leading-tight tracking-[-0.04em] text-[#0B1F3A] sm:text-[2.625rem]">What Happens Next</h2><p className="mt-3 text-base leading-7 text-[#475569] sm:text-lg">A simple and transparent process from enquiry to the right next step.</p></div>
          <div className="mt-8 grid gap-0 divide-y divide-[#DCE3EA] lg:grid-cols-3 lg:divide-x lg:divide-y-0">
            {nextSteps.map(([number, Icon, title, description]) => (
              <article className="py-6 lg:px-8 lg:first:pl-0 lg:last:pr-0" key={title}>
                ...
                <h3 className="mt-5 text-xl font-bold tracking-[-0.025em] text-[#0B1F3A]">{title}</h3>
                <p className="mt-2 text-base leading-7 text-[#475569]">{description}</p>
              </article>
            ))}
          </div>
        </div>
      ...
      <section className="bg-[#0B1F3A] px-4 py-8 text-white sm:px-6 lg:px-8" aria-labelledby="assurance-heading">
        <div className="mx-auto flex max-w-6xl flex-col gap-5 sm:flex-row sm:items-center sm:justify-between">
          <div className="max-w-2xl"><h2 id="assurance-heading" className="text-2xl font-bold tracking-[-0.035em]">Not Ready With Every Detail Yet?</h2><p className="mt-2 text-sm leading-6 text-slate-200 sm:text-base">That is completely fine. Start with what you know, and we will help you shape the rest.</p></div>
