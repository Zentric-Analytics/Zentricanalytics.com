...
export function stage6CorrectionRequestedEmail(input: TemplateInput) { return build(`Stage 6 correction requested: ${input.applicationId}`, 'Onboarding correction requested', [`${greeting(input.candidateName)} We need an update to your onboarding form.`, 'Please return to the secure candidate portal to review and resubmit Stage 6.', 'Your onboarding form will return to HR review after resubmission.']); }
export function stage6RejectedEmail(input: TemplateInput) { return build(`Onboarding update: ${input.applicationId}`, 'Onboarding update', [`${greeting(input.candidateName)} Thank you for submitting your onboarding form.`, 'Stage 6 was not approved, and further action depends on Zentric Analytics LTD.', 'This update does not imply final HR approval.']); }
export function stage7UnlockedEmail(input: TemplateInput) { return build(`Next acknowledgement stage available: ${input.applicationId}`, 'Next acknowledgement stage available', [`${greeting(input.candidateName)} Your onboarding form has been approved and the next acknowledgement stage is available.`, 'Please return to the secure candidate portal to continue the employment administration process.', 'Final HR approval is still required.']); }

export function stage7CorrectionRequestedEmail(input: TemplateInput) { return build(`Stage 7 correction requested: ${input.applicationId}`, 'Policy acknowledgement correction requested', [`${greeting(input.candidateName)} We need an update to your policy and access acknowledgements.`, 'Please return to the secure candidate portal to review HR instructions and resubmit Stage 7.', 'Final HR approval is still required before the hiring workflow is complete.']); }
export function stage7RejectedEmail(input: TemplateInput) { return build(`Policy acknowledgement update: ${input.applicationId}`, 'Policy acknowledgement update', [`${greeting(input.candidateName)} Thank you for submitting your policy and access acknowledgements.`, 'Stage 7 was not approved, and further action depends on Zentric Analytics LTD.', 'This update does not imply final HR approval.']); }
export function stage8UnlockedEmail(input: TemplateInput) { return build(`Final HR review available: ${input.applicationId}`, 'Final HR review stage available', [`${greeting(input.candidateName)} Your acknowledgements have been approved and the final HR review stage is available.`, 'Final HR approval is still required before the hiring workflow is complete.', 'Please monitor the secure candidate portal for any final HR updates.']); }

export function stage8FinalReviewCorrectionEmail(input: TemplateInput) { return build(`Final HR review update: ${input.applicationId}`, 'Final HR review update', [`${greeting(input.candidateName)} Your application is in final HR review.`, 'Zentric Analytics LTD needs to resolve a final HR review item before the workflow can be completed.', 'Please monitor the secure candidate portal and follow any instructions from HR.']); }
export function stage8RejectedEmail(input: TemplateInput) { return build(`Final HR review update: ${input.applicationId}`, 'Final HR review update', [`${greeting(input.candidateName)} Thank you for completing the hiring workflow steps.`, 'Final HR approval was not completed, and further action depends on Zentric Analytics LTD.', 'This message does not include private HR, payroll, or statutory details.']); }
export function hiringWorkflowCompletedEmail(input: TemplateInput) { return build(`Hiring workflow complete: ${input.applicationId}`, 'Hiring workflow complete', [`${greeting(input.candidateName)} Final HR approval has been completed.`, 'Your Zentric Analytics LTD hiring workflow is complete.', 'HR will contact you with next operational steps. This message does not include payroll, statutory, or private employee-file details.']); }
