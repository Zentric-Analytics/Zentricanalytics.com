...
try {
  ...
  const checks = {
    ...
    orphanRewardsOrStatements: scalar(await prisma.$queryRaw`SELECT (SELECT COUNT(*) FROM "HrBonusAward" a LEFT JOIN "HrEmployee" e ON e.id=a."employeeId" AND e."organizationId"=a."organizationId" LEFT JOIN "HrWorkRelationship" w ON w.id=a."workRelationshipId" AND w."organizationId"=a."organizationId" LEFT JOIN "HrBonusProgramVersion" p ON p.id=a."programVersionId" AND p."organizationId"=a."organizationId" WHERE e.id IS NULL OR w.id IS NULL OR p.id IS NULL)::int + (SELECT COUNT(*) FROM "HrCompStatement" s LEFT JOIN "HrEmployeeDocumentVersion" v ON v.id=s."documentVersionId" AND v."organizationId"=s."organizationId" LEFT JOIN "HrCompDecision" d ON d.id=s."decisionId" AND d."organizationId"=s."organizationId" LEFT JOIN "HrBonusAward" a ON a.id=s."awardId" AND a."organizationId"=s."organizationId" WHERE v.id IS NULL OR (s."decisionId" IS NOT NULL AND d.id IS NULL) OR (s."awardId" IS NOT NULL AND a.id IS NULL))::int AS count`),
