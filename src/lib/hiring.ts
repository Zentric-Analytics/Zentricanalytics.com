...
const limited = (label: string, max = 300) => z.string().trim().min(1, `${label} is required.`).max(max, `${label} is too long.`);
const optionalLimited = (max = 1000) => z.string().trim().max(max).optional().or(z.literal(""));
export const onboardingDocumentKinds = ["bankProof", "statutoryDocument", "additionalDocument"] as const;
export const stage6CandidateSchema = z.object({
  session: z.string().min(16, "Open the portal with a valid session."),
  fullLegalName: limited("Full legal name", 160), preferredName: optionalLimited(120), dateOfBirth: limited("Date of birth", 40), residentialAddress: limited("Residential address", 500), currentCity: limited("Current city", 120), stateOfResidence: limited("State of residence", 120), nationality: limited("Nationality", 120), phoneNumber: limited("Phone number", 80), email: z.string().trim().email("Enter a valid email address.").max(180),
  confirmedStartDate: limited("Confirmed start date", 40), preferredStartDateNotes: optionalLimited(500), workModeReadiness: limited("Work mode readiness", 500), equipmentNeeds: limited("Equipment needs", 500), internetPowerReadiness: limited("Internet/power readiness", 500), availabilityForOrientation: limited("Orientation availability", 500), emergencyStartConstraints: optionalLimited(500),
  nextOfKinName: limited("Next of kin name", 160), nextOfKinRelationship: limited("Next of kin relationship", 120), nextOfKinPhone: limited("Next of kin phone", 80), nextOfKinEmail: z.string().trim().email("Enter a valid next-of-kin email, or leave it blank.").max(180).optional().or(z.literal("")), nextOfKinAddress: optionalLimited(500), emergencyContactName: limited("Emergency contact name", 160), emergencyContactRelationship: limited("Emergency contact relationship", 120), emergencyContactPhone: limited("Emergency contact phone", 80), emergencyContactAddress: optionalLimited(500),
  bankName: limited("Bank name", 160), accountName: limited("Account name", 160), accountNumber: z.string().trim().regex(/^\d{6,20}$/, "Enter a valid account number using 6 to 20 digits."), taxIdentificationNumber: optionalLimited(80), pensionProvider: optionalLimited(160), pensionAccountNumber: optionalLimited(80), nationalIdentificationNumber: optionalLimited(80), statutoryContributionNotes: optionalLimited(1000),
  declarationAccuracy: z.literal("on", { errorMap: () => ({ message: "Confirm the accuracy declaration." }) }), payrollProcessingConsent: z.literal("on", { errorMap: () => ({ message: "Confirm payroll/statutory processing consent." }) }), employmentAdministrationConsent: z.literal("on", { errorMap: () => ({ message: "Confirm employment administration processing consent." }) }), finalApprovalAcknowledgement: z.literal("on", { errorMap: () => ({ message: "Confirm final approval acknowledgement." }) }), changeNotificationAgreement: z.literal("on", { errorMap: () => ({ message: "Confirm change notification agreement." }) }), electronicSignatureConsent: z.literal("on", { errorMap: () => ({ message: "Confirm electronic signature consent." }) }), signatureName: z.string().trim().min(2, "Type your full legal name as your electronic signature.").max(160),
});
