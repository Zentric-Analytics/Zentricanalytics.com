...
try {
  ...
  const data = { id: crypto.randomUUID(), organizationId: run.organizationId, employeeId: snapshot.employeeId, payGroupId: run.payGroupId, payrollPeriodId: run.calendarPeriodId, payrollRunId: run.id, calculationAttemptId: attempt.id, jurisdictionCode: "NG", rtaCode: "LAGOS", blockerCode: "NON_PERIODIC_PAY_RTA_RULE_REQUIRED", blockerCategory: "TAX", affectedInput: "BONUS", candidateVersion: "NG-CANDIDATE-2026.3", sourceRequirement: "Lagos RTA allocation authority", logicalKey: marker, correlationId: crypto.randomUUID() };
