...
export async function openPerformanceCycle(tx: Prisma.TransactionClient, context: PerformanceContext, input: { cycleId: string; expectedVersion: number }) {
  ...
  if (created === 0) throw new Error("This cycle has no eligible employees. Verify active employment, account, assignment, reporting line, and published job-profile mappings before opening it.");
