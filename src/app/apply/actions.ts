...
export async function submitStage1Application(_previousState: Stage1FormState, formData: FormData): Promise<Stage1FormState> {
  ...
  try {
    ...
    const email = candidateEmailTemplates.applicationReceived({ applicationId: app.applicationId, applicantName: data.fullName, portalUrl: `${process.env.APP_BASE_URL ?? process.env.NEXT_PUBLIC_SITE_URL ?? 'https://staging.zentricanalytics.com'}/track` });
