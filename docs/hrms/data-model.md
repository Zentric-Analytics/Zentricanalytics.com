...
The completion-audit migration adds explicit company-email lifecycle and preferred-notification fields, durable in-app notifications and preferences, and team-scoped supervisor assignments. These additions are nullable or safely defaulted and preserve every existing employee, recruitment, payroll, and audit row.

`HrLifecycleInstance` stores the effective or last-working date plus the offboarding reason, payroll stop date, final-payroll decision, leave-reconciliation plan, company-email disablement timestamp, and final-personal-email communication timestamp. Required versioned tasks prove asset return, access revocation, reconciliation, final payroll, exit-document creation, and final communication before completion. Completion suspends the linked account, revokes sessions and tracked system access, disables company email, and archives rather than deletes the employee.

`HrWorkflowApproval` is append-only and records the actor, actor role, request type and identifier, previous and new statuses, comment/reason, immutable timestamp, and unique correlation ID. Stage runs reserve delegation and reassignment metadata so those capabilities can be added later without rewriting approval history.
