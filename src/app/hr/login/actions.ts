...
const schema = z.object({ email: z.string().email().max(180), password: z.string().min(1).max(256) });
...
export async function hrLoginAction(formData: FormData) {
  ...
  const user = await authenticateHrCredentials(organization.id, parsed.data.email, parsed.data.password, ip);
