...
const items = [
  ...
  ["Current phase", "Repository audit and production architecture blueprint"],
  ...
  ["Tests run", "Blueprint phase: repository inspection only; no Unit 5 runtime validation claimed"],
  ...
  ["Blockers", "Implementation requires product-owner approval of the documented decisions"],
  ["Next gate", "Blueprint review and explicit Unit 5 implementation authorization"],
] as const;

export default function Unit5StatusPage() {
  return <main>
    <div className="flex flex-wrap items-start justify-between gap-4">
      <div><p className="text-sm font-semibold uppercase tracking-[0.18em] text-indigo-700">Live engineering status</p><h1 className="mt-2 text-3xl font-bold text-slate-950">Unit 5 — Leave &amp; Absence Management</h1><p className="mt-2 max-w-3xl text-slate-600">Architecture and repository-audit phase. No Unit 5 production implementation or deployment is authorized.</p></div>
      <span className="rounded-full bg-amber-100 px-4 py-2 text-sm font-bold text-amber-900">BLUEPRINT REVIEW</span>
    </div>
