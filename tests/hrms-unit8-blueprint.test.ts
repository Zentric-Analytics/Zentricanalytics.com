...
describe("Unit 8 compensation and rewards blueprint", () => {
  ...
  it("keeps the status artifact blueprint-only and free of compensation records", () => {
    const status = read("src/app/hr/admin/unit-8-status/page.tsx");
    expect(status).toContain("UNIT 8 BLUEPRINT COMPLETE — READY FOR IMPLEMENTATION APPROVAL");
    expect(status).toContain("staging and production untouched");
