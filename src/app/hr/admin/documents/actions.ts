...
export async function recordDocumentScanAction(formData: FormData) {
  ...
  await prisma.$transaction(async (tx) => {
    ...
    await tx.hrLeaveEvidence.updateMany({ where: { documentVersionId: version.id, status: "PENDING_SCAN" }, data: { status: input.status === "CLEAN" ? "SATISFIED" : input.status === "QUARANTINED" ? "REJECTED_MALWARE" : "SCAN_FAILED" } });
