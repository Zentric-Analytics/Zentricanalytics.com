...
export default async function Apply({ searchParams }: { searchParams: Promise<{ submitted?: string; error?: string }> }) {
  ...
  return <PageShell><Section eyebrow="Stage 1" title="Initial application">
    ...
    {params.error ? <p className="mb-4 rounded-xl bg-red-50 p-4 text-red-700">Please check all required fields. Enter a valid phone number for the selected country and upload a PDF, DOC, DOCX, JPG, PNG, or WEBP file up to 20MB.</p> : null}
    <form action={submitStage1Application} className="card grid gap-4 p-6 md:grid-cols-2">
      ...
      <div className="field md:col-span-2"><span>Phone</span><div className="grid gap-3 sm:grid-cols-[minmax(10.5rem,14rem)_1fr]"><select className="input w-full" name="phoneCountryIso" aria-label="Phone country" required defaultValue="NG">{countryPhoneOptions.map((country)=><option key={country.iso} value={country.iso}>{country.name} {country.dialCode}</option>)}</select><input className="input w-full" name="phoneNational" inputMode="tel" autoComplete="tel-national" pattern="[0-9+() .-]{7,}" title="Enter a valid phone number for the selected country." required /></div><span className="text-xs text-slate-500">Enter your number in national or international format. Spaces, hyphens, and parentheses are accepted when valid for the selected country.</span></div>
      ...
      <label className="field md:col-span-2">CV/resume or supporting document<input className="input" name="cv" type="file" accept={uploadAccept} required /><span className="text-xs text-slate-500">Accepted formats: PDF, DOC, DOCX, JPG, PNG, or WEBP. Maximum file size: 20MB. You can choose a file or select an image from your gallery on mobile.</span></label>
