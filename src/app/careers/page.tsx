            ...
            <Stagger className="mx-auto mt-8 grid max-w-[22rem] gap-x-3 gap-y-5 sm:max-w-[54rem] sm:grid-cols-2 lg:mt-10 lg:grid-cols-3" delay={120} staggerDelay={90}>
              {roles.map(({ Icon, title, description }) => (
                ...
                >
                  ...
                  <h3 className="mt-3 text-[18px] font-bold leading-[1.25] tracking-[-0.025em] text-[#0B1F3A] sm:text-[19px] lg:text-[20px]">{title}</h3>
                  <p className="mt-2 min-w-0 overflow-hidden text-[14px] leading-[1.55] text-[#475569] lg:text-[15px] [display:-webkit-box] [-webkit-box-orient:vertical] [-webkit-line-clamp:2]">{description}</p>
                </article>
              ))}
            </Stagger>

            <Reveal className="mx-auto mt-8 flex max-w-[36rem] flex-col items-center text-center" delay={160}>
