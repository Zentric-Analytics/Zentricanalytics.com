        ...
        {stageTwoSubmission ? (
          <div className="mt-4 space-y-5">
            <div className="grid gap-4 md:grid-cols-2">
              <div className="rounded-2xl border border-slate-200 bg-slate-50 p-4"><h3 className="font-semibold">Identity details</h3><p>Legal name: {String(stageTwoPayload.fullLegalName ?? "—")}</p><p>DOB: {String(stageTwoPayload.dateOfBirth ?? "—")}</p><p>Gender: {String(stageTwoPayload.gender ?? "—")}</p><p>Nationality: {String(stageTwoPayload.nationality ?? "—")}</p><p>Residence: {String(stageTwoPayload.currentCity ?? "—")}</p><p>Phone: {String(stageTwoPayload.applicantPhoneE164 ?? "—")}</p></div>
              <div className="rounded-2xl border border-slate-200 bg-slate-50 p-4"><h3 className="font-semibold">Primary ID summary</h3><p>Primary ID type: {String(stageTwoPayload.primaryIdType ?? "—")}</p><p>Primary ID number: {String(stageTwoPayload.primaryIdNumberMasked ?? "—")}</p><p>Authority: {String(stageTwoPayload.primaryIdIssuingAuthority ?? "—")}</p><p>Issue date: {String(stageTwoPayload.primaryIdIssueDate ?? "—")}</p><p>Expiry date: {String(stageTwoPayload.primaryIdExpiryDate ?? "—")}</p></div>
              {stageTwoPayload.hasSecondaryId ? <div className="rounded-2xl border border-slate-200 bg-slate-50 p-4"><h3 className="font-semibold">Secondary ID summary</h3><p>Secondary ID type: {String(stageTwoPayload.secondaryIdType ?? "—")}</p><p>Secondary ID number: {String(stageTwoPayload.secondaryIdNumberMasked ?? "—")}</p><p>Authority: {String(stageTwoPayload.secondaryIdIssuingAuthority ?? "—")}</p><p>Issue date: {String(stageTwoPayload.secondaryIdIssueDate ?? "—")}</p><p>Expiry date: {String(stageTwoPayload.secondaryIdExpiryDate ?? "—")}</p></div> : null}
              <div className="rounded-2xl border border-slate-200 bg-slate-50 p-4"><h3 className="font-semibold">Emergency contact</h3><p>Name: {String(stageTwoPayload.emergencyContactName ?? "—")}</p><p>Relationship: {String(stageTwoPayload.emergencyContactRelationship ?? "—")}</p><p>Phone: {String(stageTwoPayload.emergencyContactPhoneE164 ?? "—")}</p><p>Address: {String(stageTwoPayload.emergencyContactAddress ?? "—")}</p></div>
