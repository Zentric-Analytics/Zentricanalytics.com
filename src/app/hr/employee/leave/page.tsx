...
export default async function EmployeeLeavePage() {
  ...
  const [balances, requests, policyAssignments, accountPeriods] = await Promise.all([
    ...
    prisma.hrLeaveRequest.findMany({ where: { employeeId: auth.user.employee.id }, include: { leaveType: true, approvals: { include: { reviewer: true }, orderBy: { createdAt: "asc" } }, attachments: true, HrLeaveRequestVersion: { include: { evidence: { include: { documentVersion: true } } }, orderBy: { version: "desc" }, take: 1 } }, orderBy: { createdAt: "desc" }, take: 100 }),
  ...
  return <>
    ...
    <section className="mt-6 overflow-x-auto rounded-2xl bg-white p-5">
      ...
      <table className="mt-4 w-full text-left text-sm"><thead><tr className="border-b"><th className="py-3">Leave</th><th>Dates</th><th>Amount</th><th>Status</th><th>Attachment</th><th>Decision notes</th><th>Action</th></tr></thead>
        <tbody>{requests.map((request) => { const evidence = request.HrLeaveRequestVersion[0]?.evidence ?? []; return <tr className="border-b last:border-0" key={request.id}><td className="py-3">{request.leaveType.name}</td><td>{request.startDate.toLocaleDateString()} – {request.endDate.toLocaleDateString()}</td><td>{request.amount.toString()}</td><td>{request.status}</td><td>{evidence.length ? evidence.map(({ status }) => status).join(", ") : request.attachments.length ? `${request.attachments.length} legacy private file` : "—"}</td><td>{request.approvals.at(-1)?.notes ?? "—"}</td><td>{request.status === "PENDING" ? <form action={withdrawLeaveRequestAction}><input type="hidden" name="requestId" value={request.id} /><button className="font-semibold text-red-700">Withdraw</button></form> : "—"}</td></tr>; })}</tbody>
      </table>
