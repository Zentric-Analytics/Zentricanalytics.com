...
async function createSourceSnapshot(db: PrismaClient) {
  ...
  for (const handoff of handoffs) {
    const blockingRelief = await db.hrPayrollTaxReliefClaimVersion.findFirst({ where: { organizationId: organization.id, employeeId: handoff.employeeId, taxYear: 2026 } });
    if (!blockingRelief) { chosen = handoff; break; }
  }
  assert(chosen, "A staging payroll handoff without pre-existing 2026 relief claims is required.");
  ...
  const sourceManifest: Candidate2026_7Manifest = { employeeId: employee.id, workRelationshipId: relationship.id, assignmentId: assignment.id, payrollPeriodId: period.id, currency: "NGN", jurisdictionVersion: NG_2026_7_VERSION, engineVersion: "unit9-ng-2026.7", incomeEvidence, authoritativeSources: { salary: { recordId: salary.id, versionHash: "resolved-at-freeze", monthlyAmount: monthlySalary, currency: "NGN", payFrequency: "MONTHLY", effectiveFrom: salary.effectiveFrom.toISOString() }, annualization: { ruleId: "resolved-at-freeze", ruleVersion: ruleVersion + 1, frequency: "MONTHLY", periodsInTaxYear: 12, method: NG_2026_7_MONTHLY_RULE.method, taxYear: 2026, certificationStatus: "CERTIFIED" }, ytd: { sourceLedgerHash: "resolved-at-freeze", cutoff: period.cutoffAt.toISOString(), priorBonusYtd: "0", payeDeducted: "0", payeRepaid: "0", entryIds: [] }, priorEmployer: { state: "NONE", income: "0", paye: "0", payeRepaid: "0" }, deductions: { amount: "0", sourceType: "TAX_RELIEF_CLAIM_VERSIONS", sourceRecordIds: [], sourceVersions: [], evidenceReferences: [], aggregateHash: "resolved-at-freeze" } }, auditExpectedAnnualSalary: salary.amount.mul(12).toFixed(2), auditPriorBonusYtd: "0", auditPayeDeductedYtd: "0", auditPayeRepaidYtd: "0", earnings: [{ code: "SALARY", sourceType: "UNIT8", sourceId: salary.id, fixedAmount: monthlySalary, taxableBaseCode: "EMPLOYMENT", ruleVersionReference: salaryDefinition.id }], paye: { priorYtdTaxableIncome: "0", priorYtdPaye: "0", priorPayeRepaid: "0", expectedAnnualEmploymentIncome: salary.amount.mul(12).toFixed(2), eligibleAnnualDeductions: "0", periodsElapsed: 8, periodsInTaxYear: 12, currentNonPeriodicPayments: "0", priorBonusPaidTaxYearToDate: "0", priorEmployerIncome: "0", priorEmployerPaye: "0", rules: { version: "not-authority", annualizationPeriods: 12, roundingScale: 2, bands: [] } } };
...
async function main() {
  ...
  try {
    ...
    const sourceSnapshot = await db.hrPayrollInputSnapshot.findFirst({ where: { employmentIncomeBindingHash: { not: null }, sourceManifest: { path: ["jurisdictionVersion"], equals: "NG-CANDIDATE-2026.7" } }, orderBy: { createdAt: "desc" } }) ?? await createSourceSnapshot(db);
    ...
    const freeze = async (suffix: string) => { const run = await newRun(suffix); await freezeUnit9Inputs(db, maker, run.id, [{ candidate, sourceManifest: manifest as unknown as Prisma.InputJsonValue }]); return db.hrPayrollInputSnapshot.findFirstOrThrow({ where: { payrollRunId: run.id, employeeId: employee.id } }); };
    ...
    const salaryFrozen = await freeze("salary-before-append");
    ...
    const salaryReplayHash = calculateFrozenPayroll2026_7(salaryManifest, salaryFrozen.inputHash).hash;
    ...
    const ytdResult = await db.hrPayrollAuthoritativeResult.findFirstOrThrow({ where: { organizationId: sourceRun.organizationId, employeeId: employee.id }, orderBy: { createdAt: "desc" } });
    const earlyAt = new Date(sourceSnapshot.frozenAt.getTime() - 2_000);
    const lateAt = new Date(sourceSnapshot.frozenAt.getTime() + 2_000);
    const ytdRows = await Promise.all([
      db.hrPayrollYtdLedgerEntry.create({ data: { organizationId: sourceRun.organizationId, employeeId: employee.id, taxYear: period.taxYear, accumulatorCode: "BONUS", entryType: "AUTHORITATIVE", amount: new Prisma.Decimal("101"), payrollResultId: ytdResult.id, effectiveAt: earlyAt, correlationId: correlation("bonus-early") } }),
      db.hrPayrollYtdLedgerEntry.create({ data: { organizationId: sourceRun.organizationId, employeeId: employee.id, taxYear: period.taxYear, accumulatorCode: "PAYE_DEDUCTED", entryType: "AUTHORITATIVE", amount: new Prisma.Decimal("11"), payrollResultId: ytdResult.id, effectiveAt: earlyAt, correlationId: correlation("paye-early") } }),
      db.hrPayrollYtdLedgerEntry.create({ data: { organizationId: sourceRun.organizationId, employeeId: employee.id, taxYear: period.taxYear, accumulatorCode: "BONUS", entryType: "AUTHORITATIVE", amount: new Prisma.Decimal("999"), payrollResultId: ytdResult.id, effectiveAt: lateAt, correlationId: correlation("bonus-late") } }),
    ]);
    cleanup.push(() => db.hrPayrollYtdLedgerEntry.deleteMany({ where: { id: { in: ytdRows.map((row) => row.id) } } }));
    const ytdFrozen = await freeze("ytd-race");
    ...
    const reliefFrozen = await freeze("relief-v2");
    ...
    await db.hrPayrollTaxReliefClaimVersion.update({ where: { id: reliefV2.id }, data: { status: "PENDING" } });
    let pendingFailedClosed = false;
    try { await freeze("relief-pending"); } catch (error) { pendingFailedClosed = String(error).includes("ELIGIBLE_DEDUCTION_SOURCE_REQUIRED"); }
    ...
    await db.hrPayrollTaxReliefClaimVersion.update({ where: { id: reliefV2.id }, data: { status: "ELIGIBLE_FOR_PAYE_RELIEF" } });
    const annualizationFrozen = await freeze("annualization-v2");
    ...
    const priorFrozen = await freeze("prior-v2");
    ...
    assert(staleBindingRejected && authoritativeStaleResults === 0, "Stale approval produced an authoritative result.");

    const replayOne = calculateFrozenPayroll2026_7(salaryManifest, salaryFrozen.inputHash);
    const replayTwo = calculateFrozenPayroll2026_7(salaryManifest, salaryFrozen.inputHash);
