-- PL/pgSQL resolves record fields in compound boolean expressions before the
-- table-name predicate can protect them. Use nested table-specific blocks.
...
BEGIN
  ...
  END IF;
  IF TG_TABLE_NAME IN ('HrCompMarketVersion','HrCompBandVersion','HrCompPolicyVersion','HrBonusProgramVersion') THEN
    IF OLD."status"::text <> 'DRAFT' THEN RAISE EXCEPTION 'Published compensation versions are immutable'; END IF;
  END IF;
  ...
  END IF;
  IF TG_TABLE_NAME = 'HrCompDecision' AND TG_OP = 'UPDATE' THEN
    IF NEW."organizationId" <> OLD."organizationId" OR NEW."recommendationId" IS DISTINCT FROM OLD."recommendationId" OR
  ...
  END IF;
  IF TG_TABLE_NAME = 'HrCompensationRecord' AND TG_OP = 'UPDATE' THEN
    IF NEW."organizationId" <> OLD."organizationId" OR NEW."employeeId" <> OLD."employeeId" OR
