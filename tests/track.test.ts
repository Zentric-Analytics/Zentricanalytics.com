...
vi.mock("@/lib/prisma", () => ({
  prisma: {
    ...
    jobApplication: {
      findUnique: mocks.jobApplicationFindUnique,
      findFirst: mocks.jobApplicationFindFirst,
    },
    applicationAccessCode: {
      create: mocks.accessCodeCreate,
      findFirst: vi.fn(),
      update: vi.fn(),
    },
...
vi.mock("@/lib/email", () => ({
  sendAndRecordEmail: mocks.sendAndRecordEmail,
}));
vi.mock("@/lib/rate-limit", () => ({
  checkRateLimit: vi.fn(async ({ limit }) => ({
    allowed: mocks.rateLimitCount < limit,
    keyHash: "hashed-key",
    remaining: Math.max(0, limit - mocks.rateLimitCount - 1),
  })),
  hashRateLimitKey: vi.fn(() => "hashed-key"),
}));
vi.mock("@/lib/access-code-config", async () => {
  const actual = await vi.importActual<
    typeof import("../src/lib/access-code-config")
  ...
  it("Stage 2 server action creates submission, documents, signature, under-review status, and safe diagnostics", () => {
    ...
    const failureLog = actions.slice(
      failureLogStart,
      actions.indexOf("export async function submitStage3", failureLogStart),
    );
...
vi.mock("@/lib/security", async () => {
  const actual = await vi.importActual<typeof import("../src/lib/security")>(
    "../src/lib/security",
  );
...
describe("track access-code flow", () => {
  beforeEach(() => {
    ...
    mocks.jobApplicationFindUnique.mockResolvedValue({
      id: "app_db_1",
  ...
  it("keeps unknown application/email requests privacy-safe and generic", async () => {
    ...
    const { requestAccessCode } = await loadTrackActions();
    await expect(
      requestAccessCode(form("ZA-APP-2026-99999", "unknown@example.com")),
  ...
  it("returns safe error state when email recording fails for a matching application", async () => {
    mocks.sendAndRecordEmail.mockRejectedValue(
  ...
  it("creates an access code and records/sends email for a matching application/email", async () => {
    const { requestAccessCode } = await loadTrackActions();
    await expect(requestAccessCode(form())).rejects.toThrow("redirect:");
    expect(mocks.accessCodeCreate).toHaveBeenCalledWith({
      data: expect.objectContaining({
        applicationId: "app_db_1",
        codeHash: expect.any(String),
        expiresAt: expect.any(Date),
      }),
    });
    expect(mocks.sendAndRecordEmail).toHaveBeenCalledWith(
    ...
    expect(mocks.auditLogCreate).toHaveBeenCalledWith({
      data: expect.objectContaining({ action: "Access code requested" }),
    });
  });
  ...
  it("uses documented environment overrides for limits", async () => {
    ...
    const { accessCodeRateLimitConfig } =
      await import("../src/lib/access-code-config");
...
describe("admin and track UI source checks", () => {
  ...
  it("track flow separates access and verification passcode controls", () => {
    ...
    const verifyForm = readFileSync(
      "src/app/track/verify/VerifyCodeForm.tsx",
      "utf8",
    );
  ...
  it("requested and failed verification states stay on the verify page", () => {
    ...
    expect(actions).toContain(
      'status === "requested" ? "/track/verify" : "/track"',
    );
    expect(actions).toContain(
    ...
    expect(verifyForm).toContain(
      "If the details match Zentric Analytics records, a code was sent.",
    );
  ...
  it("permanent delete diagnostics avoid private fields and storage keys", () => {
    ...
    const diagnosticStart = actions.indexOf(
      "adminAction: 'permanent_delete_application'",
    );
    const diagnosticBlock = actions.slice(
      diagnosticStart,
      actions.indexOf("const adminSession", diagnosticStart),
    );
  ...
  it("offer decision redirects after persistence try/catch to avoid treating successful decisions as failures", () => {
    ...
    expect(offerBody.slice(0, catchStart)).not.toContain(
      "offer_decision_failed",
    );
  });
  it("Stage 2 server action creates submission, documents, signature, under-review status, and safe diagnostics", () => {
    const actions = readFileSync("src/app/track/actions.ts", "utf8");
