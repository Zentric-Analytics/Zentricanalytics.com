...
async function activateDueDecisions(organizationId: string, now: Date) {
  const decisions = await prisma.hrCompDecision.findMany({ where: { organizationId, status: { in: ["APPROVED", "SCHEDULED"] }, effectiveAt: { lte: now } }, orderBy: { effectiveAt: "asc" }, take: 200 });
  let activated = 0;
