...
export async function POST(request: NextRequest) {
  ...
  response.cookies.set("za_hr_invitation", parsed.data.token, { httpOnly: true, secure: process.env.NODE_ENV === "production", sameSite: "strict", maxAge: 600, path: "/hr/invitation" });
  return response;
}
