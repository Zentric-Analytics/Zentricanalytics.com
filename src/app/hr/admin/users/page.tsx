      ...
      return <article className="rounded-2xl bg-white p-5" key={user.id}>
        <div className="flex flex-wrap items-start justify-between gap-3">
          ...
          <div className="flex flex-wrap gap-2">
            {user.status === "ACTIVE" && user.id !== auth.user.id ? <form action={suspendHrUserAction}><input type="hidden" name="userId" value={user.id} /><button className="btn btn-secondary text-red-700">Suspend</button></form> : null}
        ...
        <div className="mt-4 grid gap-4 lg:grid-cols-2">
          <section>
            ...
            <div className="mt-2 flex flex-wrap gap-2">{user.roles.map(({ id, role }) => <form action={revokeHrRoleAction} key={id}><input type="hidden" name="userId" value={user.id} /><input type="hidden" name="role" value={role.key} /><button className="rounded-full bg-slate-100 px-3 py-1 text-sm">{role.name} <span aria-hidden>×</span><span className="sr-only">Revoke</span></button></form>)}</div>
            <form action={assignHrRoleAction} className="mt-3 flex gap-2"><input type="hidden" name="userId" value={user.id} /><select className="input" name="role">{roleKeys.map((role) => <option key={role}>{role}</option>)}</select><button className="btn btn-secondary">Assign</button></form>
          </section>
          <section>
