...
import { Prisma, type HrEmploymentType, type HrProbationRecommendation, type HrSeparationType } from "@prisma/client";
...
export async function rehireEmployee(tx: Prisma.TransactionClient, context: Context, input: { employeeId: string; priorRelationshipId: string; positionId: string; employmentType: HrEmploymentType; fte: number; startedAt: Date; reason: string }) { const employee = await tx.hrEmployee.findFirstOrThrow({ where: { id: input.employeeId, organizationId: context.organizationId, employmentStatus: { in: ["TERMINATED", "ARCHIVED"] } } }); const prior = await tx.hrWorkRelationship.findFirstOrThrow({ where: { id: input.priorRelationshipId, organizationId: context.organizationId, employeeId: employee.id } }); const position = await tx.hrPosition.findFirstOrThrow({ where: { id: input.positionId, organizationId: context.organizationId, status: "ACTIVE", lifecycleStatus: { in: ["OPEN", "PARTIALLY_FILLED"] } } }); const occupied = await tx.hrEmployeeAssignment.aggregate({ where: { organizationId: context.organizationId, positionId: position.id, status: "ACTIVE" }, _sum: { fte: true } }); if ((occupied._sum.fte ?? new Prisma.Decimal(0)).plus(input.fte).greaterThan(position.fullTimeEquivalent)) throw new Error("The selected rehire position does not have enough available FTE capacity."); const activeRelationshipCount = await tx.hrWorkRelationship.count({ where: { organizationId: context.organizationId, personId: prior.personId, status: { in: ["ACTIVE", "NOTICE_PERIOD", "SUSPENDED"] } } }); assertRehire({ personId: prior.personId, priorRelationshipStatus: prior.status, activeRelationshipCount, rehireOfId: prior.id }); const relationship = await tx.hrWorkRelationship.create({ data: { organizationId: context.organizationId, personId: prior.personId, employeeId: employee.id, relationshipRef: `WR-${new Date().getUTCFullYear()}-${crypto.randomUUID().slice(0, 8).toUpperCase()}`, status: "ACTIVE", startedAt: input.startedAt, rehireOfId: prior.id } }); const assignment = await tx.hrEmployeeAssignment.create({ data: { organizationId: context.organizationId, employeeId: employee.id, departmentId: position.departmentId, teamId: position.teamId, positionId: position.id, employmentType: input.employmentType, effectiveFrom: input.startedAt, status: "ACTIVE", reason: `REHIRE: ${input.reason}`, createdById: context.actorUserId, legalEntityId: position.legalEntityId, businessUnitId: position.businessUnitId, divisionId: position.divisionId, locationId: position.locationId, costCenterId: position.costCenterId, fte: input.fte, placementSnapshot: { workRelationshipId: relationship.id, source: "REHIRE" } } }); await tx.hrEmployee.update({ where: { id: employee.id }, data: { employmentStatus: "ONBOARDING", hireDate: input.startedAt, startDate: input.startedAt, terminationDate: null, terminationReason: null, archivedAt: null, companyEmailStatus: "PENDING" } }); await tx.hrEmployeeStatusHistory.create({ data: { organizationId: context.organizationId, employeeId: employee.id, previousStatus: employee.employmentStatus, newStatus: "ONBOARDING", effectiveAt: input.startedAt, reason: input.reason, changedById: context.actorUserId } }); await appendHrAudit(tx, { ...context, entityType: "HrWorkRelationship", entityId: relationship.id, action: "hr.employee.rehired", newValues: { personId: prior.personId, priorRelationshipId: prior.id, workRelationshipId: relationship.id, assignmentId: assignment.id, positionId: position.id, startedAt: input.startedAt }, reason: input.reason }); await notifyEmployee(tx, context, employee.id, { template: "hr-rehire-started", subject: "Rehire onboarding started", href: "/hr/employee/tasks", idempotencyKey: `rehire-started:${relationship.id}` }); return relationship; }
