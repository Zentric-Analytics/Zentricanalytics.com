      ...
      <section id="contact-form" className="scroll-mt-24 bg-[#F7F9FC] px-4 py-12 sm:px-6 sm:py-14 lg:px-8 lg:py-16" aria-labelledby="contact-form-heading">
        <Stagger className="mx-auto grid max-w-6xl gap-8 lg:grid-cols-[minmax(0,0.62fr)_minmax(18rem,0.38fr)] lg:gap-10" staggerDelay={100}>
          ...
          <aside className="rounded-[22px] border border-[#DCE3EA] bg-white p-6 shadow-[0_18px_48px_rgba(15,23,42,0.06)] sm:p-8">
            ...
            <Stagger className="mt-6 space-y-5 text-[#475569]" staggerDelay={80}>
              <div className="group flex gap-3"><Mail aria-hidden="true" className="mt-1 size-5 text-[#10B981]" /><div><p className="text-sm font-bold text-[#0B1F3A]">Email</p><a className="mt-1 block break-all text-base font-semibold text-[#173B67] transition-colors duration-200 ease-out hover:text-[#10B981]" href={`mailto:${directEmail}`}>{directEmail}</a></div></div>
      ...
      <section className="bg-white px-4 py-12 sm:px-6 sm:py-14 lg:px-8 lg:py-16" aria-labelledby="next-heading">
        <div className="mx-auto max-w-6xl">
          <Stagger className="max-w-3xl" staggerDelay={90}>
            ...
            <h2 id="next-heading" className="mt-3 text-[2rem] font-bold leading-tight tracking-[-0.04em] text-[#0B1F3A] sm:text-[2.625rem]">What Happens Next</h2>
            <p className="mt-3 text-base leading-7 text-[#475569] sm:text-lg">A simple and transparent process from enquiry to the right next step.</p>
          </Stagger>
      ...
      <section className="bg-[#0B1F3A] px-4 py-8 text-white sm:px-6 lg:px-8" aria-labelledby="assurance-heading">
        <div className="mx-auto flex max-w-6xl flex-col gap-5 sm:flex-row sm:items-center sm:justify-between">
          <Stagger className="max-w-2xl" staggerDelay={90}>
            <h2 id="assurance-heading" className="text-2xl font-bold tracking-[-0.035em]">Not Ready With Every Detail Yet?</h2>
            <p className="mt-2 text-sm leading-6 text-slate-200 sm:text-base">That is completely fine. Start with what you know, and we will help you shape the rest.</p>
          </Stagger>
          <Reveal delay={180}>
