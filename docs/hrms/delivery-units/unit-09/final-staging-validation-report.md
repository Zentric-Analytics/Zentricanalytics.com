...
The immutable 2026.5 candidate integrates the canonical minimum-wage decision into governed eligibility, partition approval, frozen input snapshots, calculation dispatch, PAYE-path selection, deterministic replay and finalization eligibility. Final implementation SHA `8123c4ddd7840c396042a8477f320e7a5e543f1e` is live in staging deployment `dep-da6qgo5g1s2s73a0heig`. The real staging database proved the NGN70,000 standard case as `MINIMUM_WAGE_EXEMPT` with PAYE `0.00`, held unsupported other taxable income, decision-hash-bound partitioning, idempotent replay and `NOT_CERTIFIED` finalization denial. Fresh evidence totals are 998/998 tests, 61 migrations with none pending, zero relevant integrity defects, and green TypeScript, ESLint, Prisma, build, health and readiness gates. Candidate status remains `NOT_CERTIFIED`; production was not accessed or modified.
