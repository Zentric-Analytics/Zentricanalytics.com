...
export default async function EmployeeLeavePage() {
  ...
  return <>
    ...
    <section className="mt-6 overflow-x-auto rounded-2xl bg-white p-5">
      ...
      <div className="mt-4 flex flex-wrap gap-3">{requests.flatMap((request) => request.HrLeaveRequestVersion.flatMap((version) => version.evidence.map((item) => <a className="rounded-lg border border-slate-200 px-3 py-2 text-sm font-semibold text-teal-700" href={`/api/hr/documents/versions/${item.documentVersionId}`} key={item.id}>{request.leaveType.name}: {item.documentVersion.displayFileName} ({item.status})</a>)))}</div>
    </section>
  </>;
}
