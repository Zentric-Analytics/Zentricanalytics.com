...
export async function requestTeamWorkforceEventAction(formData: FormData) {
  ...
  if (!auth.user.employee) throw new Error("A linked employee identity is required.");
  ...
  if (!employeeIds.includes(input.employeeId)) throw new Error("The employee is outside your active supervisory scope.");
  ...
  await prisma.$transaction(async (tx) => {
    const event = await createWorkforceEventDraft(tx, { organizationId: auth.user.organizationId, actorUserId: auth.user.id, actorRole: auth.roles[0] }, { employeeId: input.employeeId, type: input.type, reason: input.reason, requestedEffectiveAt: input.requestedEffectiveAt, proposedSnapshot, idempotencyKey: crypto.randomUUID(), ownerUserId: auth.user.id });
    await submitWorkforceEvent(tx, { organizationId: auth.user.organizationId, actorUserId: auth.user.id, actorRole: auth.roles[0] }, event.id, event.version);
  }, { isolationLevel: "Serializable" });
  revalidatePath("/hr/supervisor/workforce");
}
