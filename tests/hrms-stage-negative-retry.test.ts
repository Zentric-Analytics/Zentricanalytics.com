...
describe('negative decision retries preserve the first result', () => {
  it.each([5, 6, 7])('does not repeat correction/rejection writes at Stage %i', async order => {
    for (const decision of ['correction', 'reject']) {
      ...
      const tx = {
        ...
        jobApplication: { findUniqueOrThrow: async () => app, update: vi.fn().mockImplementation(async ({ data }) => Object.assign(app, data)) },
        hiringStage: { findFirstOrThrow: async () => stage, update: vi.fn().mockImplementation(async ({ data }) => Object.assign(stage, data)) },
      ...
      const action = order === 5 ? adminStage5Action : order === 6 ? adminStage6Action : adminStage7Action;
      for (let i = 0; i < 2; i++) {
