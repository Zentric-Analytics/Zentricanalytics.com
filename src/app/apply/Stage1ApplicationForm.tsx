...
function FieldError({ state, field }: { state: Stage1FormState; field: Stage1Field }) {
  ...
  return error ? <p className="text-sm font-medium text-red-700">{error}</p> : null;
}
...
function FormSection({ number, title, helper, children }: FormSectionProps) {
  return (
    <section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm md:p-7">
      ...
      <div className="grid grid-cols-1 gap-4 md:grid-cols-12 md:gap-5">{children}</div>
    </section>
  );
}
    ...
    <form action={formAction} className="space-y-6">
      ...
      >
        <div className="field md:col-span-8">
          ...
          <div className="grid gap-3 sm:grid-cols-[minmax(11rem,14rem)_1fr]">
      ...
      >
        <div className="md:col-span-12 rounded-2xl border-2 border-dashed border-slate-300 bg-slate-50 p-5 md:p-6">
          <label className="field cursor-pointer gap-3">
            ...
            <span className="text-sm leading-6 text-slate-600">Accepted formats: PDF, DOC, DOCX, JPG, JPEG, PNG, or WEBP. Maximum file size: 20MB (20,971,520 bytes). On mobile, you can upload a document or select an image from your gallery.</span>
            {selectedFile ? <span className="w-fit rounded-lg bg-white px-3 py-2 text-sm font-semibold text-brand shadow-sm">Selected: {selectedFile}</span> : null}
      ...
      >
        <label className="md:col-span-12 flex gap-3 rounded-xl border border-slate-200 bg-slate-50 p-4 leading-6">
          <input name="privacyConsent" type="checkbox" defaultChecked={values.privacyConsent === 'on'} required />
          <span>I consent to Zentric Analytics processing my application data for hiring review. <Required /><FieldError state={state} field="privacyConsent" /></span>
        </label>
        <label className="field md:col-span-7">
        ...
        <label className="md:col-span-12 flex gap-3 rounded-xl border border-slate-200 bg-slate-50 p-4 leading-6">
          ...
          <span>I confirm this typed name is my electronic signature and that the submitted information is accurate. <Required /><FieldError state={state} field="signatureConsent" /></span>
        </label>
        <div className="md:col-span-12 rounded-2xl bg-ink p-5 text-white md:flex md:items-center md:justify-between md:gap-6">
          <p className="text-sm leading-6 text-slate-200">Your upload remains private and is used only for recruitment review.</p>
          <button className="btn mt-4 w-full bg-white text-ink md:mt-0 md:w-auto" type="submit" disabled={pending}>{pending ? 'Submitting...' : 'Submit Stage 1 Application'}</button>
        </div>
      </FormSection>
    </form>
  );
